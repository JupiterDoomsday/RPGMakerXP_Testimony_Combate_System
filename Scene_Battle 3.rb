...
class Scene_Battle
  ...
  def phase3_testmony_progression(val)
    # Shift to phase 3
    @phase = 3
    #display Testimony
    ...
    case val
      when -1
        @suspect.testimony.go_to_prev_line
      when 1
        @suspect.testimony.go_to_next_line
    end
    start_phase2
  end
  ...
  def phase3_press
    ...
    #display Testimony
    @suspect = $game_troop.enemies[0]
    @suspect.is_pressed = true;
    start_phase4
    #$game_temp.message_text = "after jumping next seq #{@suspect.testimony.cur_line_count}"
  end
...
##  elsif @text_progres_window.active
  ...
  def update_phase3_item_select
    ...
    if Input.trigger?(Input::C)
      ...
      $game_troop.enemies[0].is_presenting_item
      # Play decision SE
      $game_system.se_play($data_system.decision_se)
      # Set action
